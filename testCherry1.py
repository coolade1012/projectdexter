...
#    @cherrypy.expose
#    def generate(self, pname='1'):
#	pname = pname.lower()
#        session = connect()
#        pokemon = util.get(session, tables.PokemonSpecies, identifier=pname)
#        results =  u'{0.id}\n{0.name}\n{0.genus} Pokemon\n{0.child_species}\n'.format(pokemon)
#        file = open("public/txt/dummy.txt", "w")
#        file.write(results)
#        file.close()
...
class ProjectDexterWebService(object):
     ...
     def POST(self, pname='bulbasaur'):
        ...
        file.write(results)
        file.close()
        cherrypy.session['mystring'] = results
        return results.split()
